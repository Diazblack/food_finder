defmodule FoodFinder.Data do
  ...
  def create_truck(attrs) do
    do_create_truck(attrs)
  end

  def do_create_truck(attrs \\ %{}, repo \\ Repo) do
    %Truck{}
    |> Truck.changeset(attrs)
    |> repo.insert()
  end

  def batch_create_csv_trucks(list) do
    list
    |> Enum.with_index()
    |> Enum.reduce(Multi.new(), fn {params, index}, multi ->
      multi
      |> Multi.insert({:truck, index}, Truck.changeset(%Truck{}, params))
    end)
    |> Repo.transaction()
  end
